...
if __name__ == '__main__':
    ...
    kpCv2 = []
    kpCv3 = []
    for kPoint in range(x1.shape[1]):
        kpCv1.append(cv2.KeyPoint(x1[0, kPoint], x1[1, kPoint], x1[2, kPoint]))
        kpCv2.append(cv2.KeyPoint(x2[0, kPoint], x2[1, kPoint], x2[2, kPoint]))
        kpCv3.append(cv2.KeyPoint(x3[0, kPoint], x3[1, kPoint], x3[2, kPoint]))

    matchesList12 = np.hstack((np.reshape(np.arange(0, x1.shape[1]),(x2.shape[1],1)),
                                        np.reshape(np.arange(0, x1.shape[1]), (x1.shape[1], 1)),np.ones((x1.shape[1],1))))
